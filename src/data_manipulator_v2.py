...
for row in range(len(np_df)):
    if 'happy' in np_df[row][0] or 'lively' in np_df[row][0] or 'cheerful' in np_df[row][0] or 'angry' in np_df[row][0] or 'intense' in np_df[row][0] or 'aggressive' in np_df[row][0]:
        np_df[row][0] = 'upbeat'
...
final_df.to_csv('mood_features_dataset_v4.csv')
